...
class GeminiAdapter extends BaseAdapter {
  ...
  async generate(messages, options = {}) {
    ...
    if (!apiKey) {
      ...
      if (process.env.OPENROUTER_API_KEY) {
        console.log(`[gemini-adapter] Native key missing. Delegating to OpenRouter.`);
        return llmRegistry.adapters.openrouter.generate(messages, {
          ...options,
          model: "gemini"
        });
      } else {
        console.log(`[gemini-adapter] Native and OpenRouter keys missing. Delegating to Pollinations.`);
  ...
  async stream(messages, onToken, options = {}) {
    ...
    if (!apiKey) {
      const llmRegistry = require("../llm.registry");
      if (process.env.OPENROUTER_API_KEY) {
        console.log(`[gemini-adapter] Native key missing. Delegating streaming to OpenRouter.`);
        return llmRegistry.adapters.openrouter.stream(messages, onToken, {
          ...options,
          model: "gemini"
        });
      } else {
        console.log(`[gemini-adapter] Native and OpenRouter keys missing. Delegating streaming to Pollinations.`);
